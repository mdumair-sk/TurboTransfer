...
try {
    Write-Host ""
    Write-Host "[1/5] Adding WLAN profile for '$Ssid' via netsh..." -ForegroundColor Yellow
    $addResult = netsh wlan add profile filename="$tempXmlPath" user=current interface="$InterfaceName"
    Write-Host "      $addResult"

    Write-Host ""
    Write-Host "[2/5] Disconnecting from previous network and connecting to '$Ssid'..." -ForegroundColor Yellow
    ...
    Write-Host ""
    Write-Host "[3/5] Waiting for DHCP IP on $AndroidIp subnet (192.168.49.x)..." -ForegroundColor Yellow
    $associated = $false
    $assignedIp = $null
    ...
    for ($i = 1; $i -le 35; $i++) {
        ...
        if ($ipList) {
            ...
            Write-Host "      SUCCESS: Associated! Assigned Windows IP: $assignedIp (took $elapsedSec s)" -ForegroundColor Green
            break
        } else {
            $currentIps = (Get-NetIPAddress -InterfaceAlias $InterfaceName -AddressFamily IPv4 -ErrorAction SilentlyContinue | Select-Object -ExpandProperty IPAddress) -join ", "
            Write-Host "      Waiting for 192.168.49.x lease... ($i s) [Current IPs: $currentIps]" -ForegroundColor Gray
        }
    }

    if (-not $associated) {
        Write-Host "      [WARN] 192.168.49.x address not detected after timeout. Testing socket connection to $AndroidIp..." -ForegroundColor Yellow
    }
    ...
    for ($p = 1; $p -le $PingCount; $p++) {
        ...
        try {
            ...
            $waitSuccess = $asyncConnect.AsyncWaitHandle.WaitOne(4000, $false)
            if (-not $waitSuccess) {
