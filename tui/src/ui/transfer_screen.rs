...
pub fn render_transfer_screen(f: &mut Frame, app: &AppState, area: Rect) {
    ...
    let is_completed =
        app.active_progress.as_ref().map(|p| p.status) == Some(TransferStatus::Completed);
    let is_failed = app.active_progress.as_ref().map(|p| p.status) == Some(TransferStatus::Failed);
    let is_benchmark = file_name.to_lowercase().contains("benchmark")
        || file_name.to_lowercase().contains("calibration");

    let status_str = if is_benchmark
        && app.active_progress.as_ref().map(|p| p.status) == Some(TransferStatus::InProgress)
    {
        "MULTIPATH ACTIVE"
    } else {
    ...
    let metrics_lines = if is_completed {
        ...
        let err_msg = app
            .status_message
            .as_deref()
            .unwrap_or("Transfer interrupted or rejected by peer");
        vec![
            Line::from(""),
            Line::from(vec![Span::styled(
                "   ✖ File transfer failed to complete",
            ...
            Line::from(""),
            Line::from(vec![Span::styled(
                "   [R] Retry / Resume   │   [Esc] Dashboard   │   [D] Diagnostics Details",
                Style::default().fg(Color::DarkGray),
            )]),
        ]
    } else {
        vec![
            ...
            Line::from(vec![
                ...
                Span::styled(
                    ...
                    Style::default().fg(if usb_mbps > 0.01 {
                        Color::Green
                    } else {
                        Color::DarkGray
                    }),
            ...
            Line::from(vec![
                ...
                Span::styled(
                    ...
                    Style::default().fg(if wifi_mbps > 0.01 {
                        Color::Green
                    } else {
                        Color::DarkGray
                    }),
            ...
            Line::from(""),
            Line::from(vec![Span::styled(
                "   [P] Pause  │  [R] Resume  │  [C] Cancel  │  [D] Details  │  [Esc] Back",
                Style::default().fg(Color::DarkGray),
            )]),
        ]
    };
