...
pub fn render_transfers(f: &mut Frame, app: &AppState, area: Rect) {
    ...
    let tabs = Tabs::new(titles)
        .block(Block::default().borders(Borders::ALL).border_type(BorderType::Rounded).title(" Transfer Filter "))
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(format!("{:<30}", "FILE NAME"), Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        Span::styled(format!("{:<15}", "SIZE"), Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        Span::styled(format!("{:<12}", "ROLE"), Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        Span::styled(format!("{:<16}", "STATUS"), Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        Span::styled("TRANSFER ID", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
    ]));
    ...
    if filtered.is_empty() {
        lines.push(Line::from(Span::styled("   No transfers in this category.", Style::default().fg(Color::DarkGray))));
    } else {
        for (i, t) in filtered.iter().enumerate() {
            ...
            let role_str = match t.role {
                turbotransfer_core::manifest::TransferRole::Sender => "Sender",
                turbotransfer_core::manifest::TransferRole::Receiver => "Receiver",
            };

            let (status_str, status_style) = match t.status {
                TransferStatus::InProgress => ("IN_PROGRESS", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                TransferStatus::Completed => ("COMPLETED", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(format!("{:<15}", size_str), Style::default().fg(Color::Gray)),
